...
class Core(CoreLogic):
    ...
    async def backup(self, ctx, backup_path: str = None):
        ...
        if data_dir.exists():
            ...
            print("Backup complete. Destination:", backup_file)

            await ctx.send(
                _("A backup has been made of this instance. It is at {}.").format(backup_file)
                + "\n"
                + _("Would you like to receive a copy via DM? (y/n)")
            )

            filesize_limit = 8 * 1024 * 1024
            buf_size = 16 * 1024
            num_parts = ceil(backup_file.stat().st_size / filesize_limit)

            if num_parts > 1:
                bytes_copied = 0
                split_backup_files = []

                with backup_file.open("rb") as src:
                    for i in range(num_parts):
                        stop_copying_at = (i + 1) * filesize_limit
                        part_file = Path(backup_pth / "{}.part{}".format(backup_filename, i))
                        split_backup_files.append(part_file)

                        with part_file.open("wb") as dest:
                            while bytes_copied <= stop_copying_at:
                                buf = src.read(buf_size)
                                bytes_written = dest.write(buf)
                ...
                if msg.content.lower().strip() == "y":
                    if num_parts > 1:
                        await ctx.author.send(
                            _("The backup has been split into {} parts:").format(num_parts)
                        )
                        for part_file in split_backup_files:
                            await ctx.author.send(file=discord.File(str(part_file)))
                    else:
                        await ctx.author.send(
